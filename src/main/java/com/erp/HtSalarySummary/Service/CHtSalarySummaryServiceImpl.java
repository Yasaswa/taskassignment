...
import com.erp.HtSalarySummary.Repository.IHtSalaryDeductionRepository;
import com.erp.HtSalarySummary.Repository.IHtSalaryEarningsRepository;
import com.erp.HtSalarySummary.Repository.IHtSalarySummaryRepository;
...
@Service
public class CHtSalarySummaryServiceImpl implements IHtSalarySummaryService {
    ...
    @Autowired
    private IHtSalarySummaryRepository iHtSalarySummaryRepository;
    @Autowired
    private IHtSalarySummaryViewRepository iHtSalarySummaryViewRepository;
    @Autowired
    private IHtSalaryEarningsRepository iHtSalaryEarningsRepository;
    @Autowired
    private IHtSalaryDeductionRepository iHtSalaryDeductionRepository;
    @Autowired
    private IHtAttendanceMonthlyRepository iHtAttendanceMonthlyRepository;
    @Autowired
...
//                        (item.getJob_type_id().equals(jobCodeId)));
    ...
    public Map<String, Object> FnAddUpdateRecord(JSONObject json, int company_id) {
        ...
        try {
            ...
            List<CHtSalarySummaryModel> allEmplSalarySummaryModels = objectMapper.readValue(allEmplSalarySummaryData.toString(), new TypeReference<List<CHtSalarySummaryModel>>() {
            });
            // Collecting employee IDs into a list
            List<Integer> employeeIds = allEmplSalarySummaryModels.parallelStream().map(CHtSalarySummaryModel::getEmployee_id) // Map each object to its employee ID
                    .collect(Collectors.toList());

            // Update the old records.
            int numberOfUpdatedSummaryRows = iHtSalarySummaryRepository.deletePreviousRecords(employeeIds, salaryForYear, salaryForMonth, company_id, allEmplSalarySummaryModels.get(0).getCreated_by());
            iHtSalaryEarningsRepository.deletePreviousRecords(employeeIds, salaryForYear, salaryForMonth, company_id, allEmplSalarySummaryModels.get(0).getCreated_by());
            iHtSalaryDeductionRepository.deletePreviousRecords(employeeIds, salaryForYear, salaryForMonth, company_id, allEmplSalarySummaryModels.get(0).getCreated_by());

            allEmplSalarySummaryModels.forEach(employeeSalarySummary -> {
                employeeSalarySummary.setSalary_transaction_id(0);
            });
            List<CHtSalarySummaryModel> savedAllEmplSalarySummaryModels = iHtSalarySummaryRepository.saveAll(allEmplSalarySummaryModels);

            List<CHtSalaryEarningsModel> allEmplSalaryEarningsModels = objectMapper.readValue(allEmplEarningsSalaryData.toString(), new TypeReference<List<CHtSalaryEarningsModel>>() {
            });
            List<CHtSalaryEarningsModel> allEmplSalaryEarningsModelsForSave = new ArrayList<>();

            List<CHtSalaryDeductionModel> allEmplSalaryDeductionsModels = objectMapper.readValue(allEmplDeductionsSalaryData.toString(), new TypeReference<List<CHtSalaryDeductionModel>>() {
            });
            List<CHtSalaryDeductionModel> allEmplSalaryDeductionsModelsForSave = new ArrayList<>();
            ...
            savedAllEmplSalarySummaryModels.forEach(employeeSavedSalarySummary -> {
                ...
                List<CHtSalaryEarningsModel> employeeSalaryEarnings = allEmplSalaryEarningsModels.parallelStream().filter(e -> e.getEmployee_id() == employeeId).map(earning -> {
                ...
                List<CHtSalaryDeductionModel> employeeSalaryDeductions = allEmplSalaryDeductionsModels.parallelStream().filter(d -> d.getEmployee_id() == employeeId).map(deduction -> {
            ...
            iHtSalaryEarningsRepository.saveAll(allEmplSalaryEarningsModelsForSave);
            iHtSalaryDeductionRepository.saveAll(allEmplSalaryDeductionsModelsForSave);
            ...
            if (e.getRootCause() instanceof SQLException) {
                ...
                amApplicationErrorLogsServiceImpl.addErrorLog(company_id, "api", "/api/HtSalarySummary/FnAddUpdateRecord", sqlEx.getErrorCode(), sqlEx.getMessage());
            ...
            e.printStackTrace();
            amApplicationErrorLogsServiceImpl.addErrorLog(company_id, "api", "/api/HtSalarySummary/FnAddUpdateRecord", 0, e.getMessage());
            responce.put("success", "0");
            responce.put("data", "");
            responce.put("error", e.getMessage());
        }
    ...
    public Object FnDeleteRecord(int salary_transaction_id, int company_id) {
        Optional<CHtSalarySummaryModel> option = iHtSalarySummaryRepository.findById(salary_transaction_id);
        CHtSalarySummaryModel cHtSalarySummaryModel = new CHtSalarySummaryModel();
        if (option.isPresent()) {
            cHtSalarySummaryModel = option.get();
            cHtSalarySummaryModel.setIs_delete(true);
            cHtSalarySummaryModel.setDeleted_on(new Date());
            iHtSalarySummaryRepository.save(cHtSalarySummaryModel);
        }
    ...
    public Map<String, Object> FnShowParticularRecordForUpdate(int salary_transaction_id, int company_id) {
        ...
        try {
            ...
            List<Map<String, Object>> salaryEarningsModels = iHtSalaryEarningsRepository.FnGetMonthlyEarningBySalarySummaryTransId(salary_transaction_id, company_id);
            List<Map<String, Object>> salaryDeductionModels = iHtSalaryDeductionRepository.FnGetMonthlyDeductionBySalarySummaryTransId(salary_transaction_id, company_id);
...
//		// ** Get Monthly attendance as per the employee code month, year, companyId
        List<CHtAttendanceMonthlyModel> allEmplMonthlyAttendanceList = iHtAttendanceMonthlyRepository.FnGetMonthlyAttendanceByEmplCodes(employeeCodes, salaryForMonthNumber, salaryForYear, company_id);

//		// ** Get Monthly JobTypeWise attendance as per the employee code, month, year, companyId; Only Applicable For Workers;
        List<CHtAttendanceMonthlyJobTypeModel> allEmplJobTypeCodeWiseMonthlyAttList = iHtAttendanceMonthlyJobTypeRepository.getJobTypeWiseMonthlyRecordsByMonth(employeeCodes, salaryForMonthNumber, salaryForYear, company_id);
//      // ** All JobTypes;
...
//			        //******** It will storing all Heads means Earning Nd Deductions
                    ...
                    String currentEmplShift = employee.getShift_start_end_time();
                    ...
                    CHtAttendanceMonthlyModel currentEmplMonthlyAttendance = allEmplMonthlyAttendanceList.stream().filter(attendance -> attendance.getEmployee_id() == currentEmplId && currentEmplCode.equals(attendance.getEmployee_code())).findFirst().orElse(null);
...
//                      It will getting from attendance but now not getting properly so we calculate it here.
                        ...
                        if (currentEmplTypeGroup.equals("Staffs")) {
                            ...
                            if (currentEmplType.equals("Semi-Staff") && !employee.isAttendance_exclude_flag()) {
...
//                   // *******************************************************************************************************************************************
                            ...
                            f_totalSalaryDays = (currentEmplMonthlyAttendance != null) ? currentEmplMonthlyAttendance.getPresent_days() + currentEmplMonthlyAttendance.getHalf_days() : 0.0;
...
//                  // ************************ Worker JobTypeWise Salary Changes After 02/08/2024 Start's **********************************************************
                            Map<String, Double> stringDoubleMap = FnCalculateWrkSalaryJobTypeCodeWise(employee, currentEmplMonthlyAttendance,
                                    allEmplJobTypeCodeWiseMonthlyAttList, earningHeadsNameMap, combinedHeadsNameMap, allJobTypeDetails, hrPayrollSettings);
...
//				// ********* Salary calculation starts For Deductions;
                        ...
                        if (pfIsApplicable && !currentEmplTypeGroup.equals("Workers")) {
                            ...
                            if (pfAmountLimit.get() != 0
//                                    && c_basic < pfAmountLimit.get()
                            ) {
...
//				// ** Also add the monthly deductions(as per monthly deduction upload).
                        ...
                        deductionHeadsNameMap.put("Total Deduction", FnGetCeilingValue(c_totalDeductions));
                        combinedHeadsNameMap.put("Total Deduction", FnGetCeilingValue(c_totalDeductions));
...
//                emplSalaryDtl.put("PerHr Salary", formatDecimalVal.format(0.0));
    ...
    private Map<String, Double> FnCalculateWrkSalaryJobTypeCodeWise(CEmployeesViewModel employee, CHtAttendanceMonthlyModel currentEmplMonthlyAttendance,
                                                                    List<CHtAttendanceMonthlyJobTypeModel> allEmplJobTypeCodeWiseMonthlyAttList,
                                                                    HashMap<String, Object> earningHeadsNameMap, HashMap<String, Object> combinedHeadsNameMap,
                                                                    List<CJobTypeModel> allJobTypeDetails, CHmHrpayrollSettingsModel hrPayrollSettings) {
        ...
        String shiftStartAndEndTime = employee.getShift_start_end_time(); // Assuming you have the employee object
        ...
        Double c_totalEarning = 0.0, c_calculatedWrkSalary = 0.0,
                c_attendanceAllow = 0.0, c_nightAllow = 0.0, c_specialAllow = 0.0, c_otherAllow = 0.0,
...
//        Double totalPresentDays = currentEmplMonthlyAttendance.getPresent_days() + currentEmplMonthlyAttendance.getHalf_days();
        ...
        c_MonthlyWorkedHrs = currentEmplMonthlyAttendance.getMonthly_hours();
        Double totalHours = totalShiftHours * totalPresentDays;
        // Check if the monthly worked hours exceed the total shift hours

        // if worker how many hour work then take that much salary not how much day present (suggested by Rathore sir)

//        if (c_MonthlyWorkedHrs > totalHours) {
...
// Attendance allowance for 24 and 26 days And on Time Allowance ************************
        double earlyDayCount = 0;

        Double maxAttendanceAllowanceDays = hrPayrollSettings.getAttendance_max_allow_days();
        Double minAttendanceAllowanceDays = hrPayrollSettings.getAttendance_min_allow_days();

        if (maxAttendanceAllowanceDays != 0 && minAttendanceAllowanceDays != 0) {
            if (totalPresentDays >= minAttendanceAllowanceDays && totalPresentDays < maxAttendanceAllowanceDays) {
                // Between min and just below max → 24-day logic
                c_attendanceAllow = currentEmplJobType.getJob_type_attendance_allow_24_days() * totalPresentDays;
                earningHeadsNameMap.put("Attend Allow", formatDecimalVal.format(c_attendanceAllow));
                combinedHeadsNameMap.put("Attend Allow", formatDecimalVal.format(c_attendanceAllow));

            } else if (totalPresentDays >= maxAttendanceAllowanceDays) {
                // Greater than or equal to max → 26-day logic
                c_attendanceAllow = currentEmplJobType.getJob_type_attendance_allow_26_days() * totalPresentDays;
                earningHeadsNameMap.put("Attend Allow", formatDecimalVal.format(c_attendanceAllow));
                combinedHeadsNameMap.put("Attend Allow", formatDecimalVal.format(c_attendanceAllow));
            }
        ...
        String shiftStartStr = shiftStartAndEndTime.split("-")[0]; // "08:00:00"
        LocalTime shiftStartTime = LocalTime.parse(shiftStartStr);

// Shift time minus 15 minutes
        LocalTime shiftStartMinus15 = shiftStartTime.minusMinutes(15);

        for (int day = 1; day <= 31; day++) {
            String methodName = "getIn_out_time" + day;
            try {
                String inOut = (String) CHtAttendanceMonthlyModel.class.getMethod(methodName).invoke(currentEmplMonthlyAttendance);
                if (inOut != null && inOut.contains("/")) {
                    String[] timeParts = inOut.split("/");
                    String inTimeStr = timeParts[0]; // e.g. "07:45"
                    String outTimeStr = timeParts[1]; // e.g. "19:45"

                    // Check both in and out times are not "00:00"
                    if (!inTimeStr.equals("00:00") && !outTimeStr.equals("00:00")) {
                        // Add seconds if needed
                        if (inTimeStr.length() == 5) inTimeStr += ":00";
                        LocalTime inTime = LocalTime.parse(inTimeStr);

                        // Check if inTime is 15+ mins earlier than shiftStart
                        if (!inTime.isAfter(shiftStartMinus15)) {
                            earlyDayCount++;
                        }
                    }
                }
...
// Multiply early days by 30
        double earlyAllowance = earlyDayCount * currentEmplJobType.getOn_time_allowance();

// If needed, add this to your map or variable
        c_otherAllow = earlyAllowance;
        earningHeadsNameMap.put("Other Allowance", formatDecimalVal.format(c_otherAllow));
        combinedHeadsNameMap.put("Other Allowance", formatDecimalVal.format(c_otherAllow));
...
//// ***************** Function to evaluate the Expressions (Without Ternary Operator)
//public static Double FnEvaluateExpression(String expression) {
//	try {
//		char[] tokens = expression.toCharArray();
//		Stack<Double> values = new Stack<>();
...
//				return 0.0; // Return 0 if an invalid character is encountered
//			}
//		}
//
//		while (!ops.empty())
...
//	} catch (Exception e) {
//		System.out.println("Error in expression...!");
//		return 0.0; // Return 0 for any other exception
//	}
//}
//
//public static boolean hasPrecedence(char op1, char op2) {
