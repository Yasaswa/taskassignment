...
public interface IPtGoodsReceiptPaymentTermsRepository extends JpaRepository<CPtGoodsReceiptPaymentTermsModel, Integer> {
	...
	@Modifying
	@Transactional
	@Query(value = "update pt_goods_receipt_payment_terms set is_delete = 1, deleted_on = CURRENT_TIMESTAMP(), deleted_by = ?2 where goods_receipt_master_transaction_id=?1 and is_delete=0", nativeQuery = true)
	void deletePaymentTerms(int goods_receipt_master_transaction_id, String UserName);
