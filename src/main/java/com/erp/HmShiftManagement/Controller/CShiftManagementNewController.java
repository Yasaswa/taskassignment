...
import com.erp.HtAttendaceNew.Repository.IHtAttendanceDailyNewRepository;
...
@RestController
@RequestMapping(value = "/api/shiftManagementnew")
public class CShiftManagementNewController {

	private static final int PRESENT = 1;
	private static final int ABSENT = 2;
	private static final int HALF_DAY = 3;
	private static final int OUT_DOOR_DUTY = 4;
	...
	@Autowired
	IHtAttendanceDailyNewRepository iHtAttendanceDailyNewRepository;
	@Autowired
	...
	@Autowired
	IHtAttendanceMonthlyNewRepository iHtAttendanceMonthlyNewRepository;
	@Autowired
	...
	public static void setPresentyDateWiseForMonthlyCalculations(CHtAttendanceDailyNewModel empAtt, CHtAttendanceMonthlyNewModel htAttendanceMonthlyModel, int day, List<Map<String, Object>> leaveApplicationDetails) {
		try {
...
//			CALCULATE TOTAL HOURS
			htAttendanceMonthlyModel.setMonthly_hours(
					empAtt.getWorking_minutes() != null
							? htAttendanceMonthlyModel.getMonthly_hours() + parseTimeToHours(empAtt.getWorking_minutes())
	...
	public static void setWeeklyOffDays(CHtAttendanceMonthlyNewModel htAttendanceMonthlyModel, int year, int month, String weeklyoffName, List<String> thisMonthHolidaysDates, boolean attendance_exclude_flag) {
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
...
//               // Removed the condition for auto_attendace_flag;(Because for the also set the holiday)
				if (currentDay.getDayOfWeek().name().equalsIgnoreCase(weeklyoffName) && (!holidayDatesSet.contains(currentDayString))) {
					weeklyOffCount++;
				}
			}
		} else {
...
//                }
				// Removed the condition for auto_attendace_flag;(Because for the also set the holiday)
				if (date.getDayOfWeek().name().equalsIgnoreCase(weeklyoffName) && (!holidayDatesSet.contains(currentDayString))) {
					weeklyOffCount++;
				}
			}
		}
	...
	public List<CHtShiftRosterNewModel> FnGetNewShiftRosterDetails(List<String> employeeCodes, List<Integer> months, List<Integer> years) {
		List<CHtShiftRosterNewModel> employeeShiftDetails = iHtShiftRosterNewRepository.FnGetShiftRosterDetails(employeeCodes, months, years);
	...
	private Map<String, String> getNewShiftDetailsForDay(CHtShiftRosterNewModel shiftRoster, int day) {
		Map<String, String> shiftDetails = new HashMap<>();
		try {
			Field shiftIdField = CHtShiftRosterNewModel.class.getDeclaredField("shift_id" + day);
			Field shiftNameField = CHtShiftRosterNewModel.class.getDeclaredField("shift_name" + day);
			shiftIdField.setAccessible(true);
			shiftNameField.setAccessible(true);
			Integer shiftId = (Integer) shiftIdField.get(shiftRoster);
			String shiftName = (String) shiftNameField.get(shiftRoster);
			shiftDetails.put("shift_id", String.valueOf(shiftId));
			shiftDetails.put("shift_name", shiftName);
		} catch (NoSuchFieldException | IllegalAccessException e) {
			e.printStackTrace();
			shiftDetails.put("shift_id", null);
	...
	public Map<String, Object> FnShowEmployeeDetails(@RequestBody CEmployeeWiseAttendanceRequest employeeWiseAttendanceRequest) {
		...
		try {
			...
			AtomicBoolean current_two_day_shift = new AtomicBoolean(false);
...
//			GET MONTHS AND YEARS LIST FOR SHIFT ROSTER DETAILS INFO
			...
			queryBuilder.append("SELECT ")
					...
					.append("shift_name, ").append("current_shift_name, ")
					...
					.append("current_shift_start_end_time, ")
					.append("current_shift_grace_hours_min, ")
					.append("current_shift_grace_hours_max, ")
					.append("current_halfday_hours, ")
					.append("current_fullday_hours, ")
					.append("current_grace_early_time, ")
					.append("current_grace_late_time, ")
					.append("current_two_day_shift, ")
					.append("attendance_exclude_flag, ")
					.append("department_id, ")
					.append("shift_id, ").append("current_shift_id, ")
...
//			GET SHIFT DETAILS FROM ht_shift_roster
			...
			List<CHtShiftRosterNewModel> getNewShiftRosterDetails = FnGetNewShiftRosterDetails(getEmployeeCodes, monthList, yearList);  // GET NEW SHIFT ROSTER DETAILS
...
//						.filter(item -> item.getEmployee_code().equals("W21015"))
...
//                                  PROCESS THE MIN AND MAX DATES AS NEEDED
									...
									String actualWorkedHoursFormatted = "";
									if (minDateTime.isPresent() && maxDateTime.isPresent()) {
										Duration actualWorkedDuration = Duration.between(minDateTime.get(), maxDateTime.get());
										long hoursWorked = actualWorkedDuration.toHours();
										long minutesWorked = actualWorkedDuration.toMinutes() % 60;  // Remainder minutes after hours
//                                      Format the worked hours and minutes as HH:mm
										actualWorkedHoursFormatted = String.format("%02d:%02d", hoursWorked, minutesWorked);
									}

//									GET THE CURRENT SHIFT
//									Integer current_shift_id = employeeItem.getCurrent_shift_id();
									Double current_halfday_hours = employeeItem.getCurrent_halfday_hours();
									Double current_fullday_hours = employeeItem.getCurrent_fullday_hours();
									Double current_grace_early_time = employeeItem.getCurrent_grace_early_time();
									Double current_grace_late_time = employeeItem.getCurrent_grace_late_time();
									String current_shift_start_end_time = employeeItem.getCurrent_shift_start_end_time();
									Integer current_shift_grace_hours_min = employeeItem.getCurrent_shift_grace_hours_min();
									Integer current_shift_grace_hours_max = employeeItem.getCurrent_shift_grace_hours_max();
									current_two_day_shift.set(employeeItem.isCurrent_two_day_shift());

//						            CHECK IF THE EMPLOYEE IS IN THE NEW SHIFT ROSTER DETAILS
									Optional<CHtShiftRosterNewModel> optionalNewShiftRoster = getNewShiftRosterDetails.stream()
											.filter(roster -> roster.getEmployee_code().equals(employeeItem.getEmployee_code())
													&& roster.getAttendance_year().equals(year)
													&& roster.getAttendance_month().equals(month))
											.findFirst();

									if (optionalNewShiftRoster.isPresent()) {
										CHtShiftRosterNewModel shiftRoster = optionalNewShiftRoster.get();
										int dayOfMonth = date.getDayOfMonth();
										Map<String, String> newShiftDetailsForDay = getNewShiftDetailsForDay(shiftRoster, dayOfMonth);


//							FIND SHIFT DEATILS NOW FROM SHIFT MASTER ONLY IF EMPLOYEE SHIFT DIFFERENT FROM EMPLOYEE MASTER
										if (newShiftDetailsForDay.get("shift_id") != null && !String.valueOf(shift_id).equals(newShiftDetailsForDay.get("shift_id"))) {
											CShiftModel shiftModel = getShiftDetails.parallelStream()
													.filter(shift -> String.valueOf(shift.getShift_id()).equals(newShiftDetailsForDay.get("shift_id"))).findFirst().get();

											current_halfday_hours = shiftModel.getHalfday_hours();
											current_fullday_hours = shiftModel.getFullday_hours();
											current_grace_early_time = Double.valueOf(shiftModel.getGrace_early_time());
											current_grace_late_time = Double.valueOf(shiftModel.getGrace_late_time());
											current_shift_grace_hours_min = shiftModel.getShift_grace_hours_min();
											current_shift_grace_hours_max = shiftModel.getShift_grace_hours_max();
											current_shift_start_end_time = shiftModel.getStart_time() + "-" + shiftModel.getEnd_time();
											current_two_day_shift.set(shiftModel.isTwo_day_shift());

											employeeMap.put("shift_name", shiftModel.getShift_name());
											employeeMap.put("shift_start_end_time", current_shift_start_end_time);
											employeeMap.put("halfday_hours", current_halfday_hours);
											employeeMap.put("fullday_hours", current_fullday_hours);
											employeeMap.put("grace_early_time", current_grace_early_time);
											employeeMap.put("grace_late_time", current_grace_late_time);
											employeeMap.put("two_day_shift", current_two_day_shift.get());
											employeeMap.put("shift_present", shiftModel.getShift_name());
											employeeMap.put("shift_grace_hours_min", current_shift_grace_hours_min);
											employeeMap.put("shift_grace_hours_max", current_shift_grace_hours_max);
										}

									}


//                                  Assume shiftStartTime and min_att_time are LocalTime objects
									LocalTime minAttTime = minDateTime.isPresent() ? minDateTime.get().toLocalTime() : null;
									LocalTime maxAttTime = maxDateTime.isPresent() ? maxDateTime.get().toLocalTime() : null;

									// Split the current shift start-end time to get the start time
									String[] currentShiftTimes = current_shift_start_end_time.split("-");
									LocalTime currentShiftStartTime = LocalTime.parse(currentShiftTimes[0].trim(), DateTimeFormatter.ofPattern("HH:mm:ss"));
									LocalTime currentShiftEndTime = LocalTime.parse(currentShiftTimes[1].trim(), DateTimeFormatter.ofPattern("HH:mm:ss"));

									// Parse actual worked hours if available
									if (!actualWorkedHoursFormatted.isEmpty()) {
										String[] workedHoursParts = actualWorkedHoursFormatted.split(":");
										int workedHours = Integer.parseInt(workedHoursParts[0]);
										int workedMinutes = Integer.parseInt(workedHoursParts[1]);

										// Calculate total worked minutes
										long totalWorkedMinutes = workedHours * 60 + workedMinutes;

										// Define the threshold for a full 8-hour workday (in minutes)
										long fullDayMinutes = 8 * 60;

										// If the employee worked less than 8 hours
										if (totalWorkedMinutes < fullDayMinutes) {
											// Calculate the difference in minutes
											long shortfallMinutes = fullDayMinutes - totalWorkedMinutes;

											// Adjust the current shift start and end times by reducing the shortfall
											LocalTime adjustedMinAttTime = currentShiftStartTime;
											LocalTime adjustedMaxAttTime = currentShiftEndTime.minusMinutes(shortfallMinutes);

											// Ensure adjusted times are within valid shift times
											if (adjustedMinAttTime.isAfter(currentShiftEndTime)) {
												adjustedMinAttTime = currentShiftEndTime;
											}
											if (adjustedMaxAttTime.isBefore(currentShiftStartTime)) {
												adjustedMaxAttTime = currentShiftStartTime;
											}

											minAttTime = adjustedMinAttTime;
											maxAttTime = adjustedMaxAttTime;


											// Update the min_att_time and max_att_time in the employeeMap
											employeeMap.put("min_att_time", adjustedMinAttTime.format(DateTimeFormatter.ofPattern("HH:mm")));
											employeeMap.put("max_att_time", adjustedMaxAttTime.format(DateTimeFormatter.ofPattern("HH:mm")));
										}else{
											minAttTime = currentShiftStartTime;
											maxAttTime = currentShiftEndTime;

											employeeMap.put("min_att_time", currentShiftStartTime);
											employeeMap.put("max_att_time", currentShiftEndTime);
										}
									}else{
										minAttTime = minDateTime.isPresent() ? currentShiftStartTime : null;
										maxAttTime =  maxDateTime.isPresent() ? currentShiftEndTime : null;

										// Update the min_att_time and max_att_time in the employeeMap
										employeeMap.put("min_att_time", minDateTime.isPresent() ? currentShiftStartTime : null);
										employeeMap.put("max_att_time", maxDateTime.isPresent() ? currentShiftEndTime : null);
									}



									LocalDateTime adjustedMinDateTime = Optional.ofNullable(minAttTime)
											.map(time -> LocalDateTime.of(LocalDate.parse(attendanceDate), time))
											.orElse(null);

									LocalDateTime adjustedMaxDateTime = Optional.ofNullable(maxAttTime)
											.map(time -> LocalDateTime.of(
													current_two_day_shift.get() ?
															LocalDate.parse(attendanceDate).plusDays(1) :
															LocalDate.parse(attendanceDate), time))
											.orElse(null);



//									GET PUNCH FROM MISS PUNCH CORRECTION IF AVAILABLE
									FnCheckMissPunchCorrection(employeeMap, attendanceMisspunchCorrection, Optional.ofNullable(adjustedMinDateTime), Optional.ofNullable(adjustedMaxDateTime));

									if (minDateTime.isPresent() && maxDateTime.isPresent()) {
										// Now you can calculate the worked hours using these LocalDateTime values
										Duration workedDuration = Duration.between(adjustedMinDateTime, adjustedMaxDateTime);
										long hoursWorked = workedDuration.toHours();
										long minutesWorked = workedDuration.toMinutes() % 60;  // Remainder minutes after hours

										// Format the worked hours and minutes as HH:mm
										String workedHoursFormatted = String.format("%02d:%02d", hoursWorked, minutesWorked);

										// Store the calculated worked hours in the employeeMap
										employeeMap.put("worked_hours", workedHoursFormatted);

										// DETERMINE EMPLOYEE PRESENT STATUS
										determineEmployeePresenceStatus(current_grace_early_time, current_grace_late_time, current_fullday_hours, current_halfday_hours,
												employeeItem.getEmployee_code(), workedDuration, weeklyOffName, dayName, attendanceDate, leaveApplicationDetails,
												attendanceStatusDetails, isHoliday, current_two_day_shift, employeeMap, employeeItem.isAttendance_exclude_flag(), shortLeaveApplicationDetails, hrPayrollSettings);

										// EXTRA REMARK REGARDING EMPLOYEE PRESENCE
										determineEmployeeExtraRemark(current_grace_early_time, current_grace_late_time, employeeItem.getEmployee_code(), adjustedMinDateTime,
												adjustedMaxDateTime, currentShiftStartTime, currentShiftEndTime, shortLeaveApplicationDetails, employeeMap, attendanceDate);
									}
...
//													&& isHoliday == 0
							...
							// Set The JobTypeCode;
							// Stream to find the first occurrence with the target employee code
	...
	private void FnCheckMissPunchCorrection(Map<String, Object> employeeMap, List<Map<String, Object>> attendanceMisspunchCorrection, Optional<LocalDateTime> minDateTime, Optional<LocalDateTime> maxDateTime) {
	...
	private void determineEmployeeExtraRemark(Double grace_early_time, Double grace_late_time, String employee_code, LocalDateTime minDateTimeValue,
		...
		if (shortLeaveStatus) {
			...
			if (currentRemark != null && !currentRemark.isEmpty()) {
				...
				employeeMap.put("present_extra_remark", "S");
			}
...
//			OTHERWISE HE IS ABSENT
	...
	private Map<String, Object> convertEmployeeItemToMap(CEmployeesViewModel employeeItem) {
		...
		map.put("shift_name", employeeItem.getCurrent_shift_name());
		...
		map.put("shift_start_end_time", employeeItem.getCurrent_shift_start_end_time());
		map.put("halfday_hours", employeeItem.getCurrent_halfday_hours());
		map.put("fullday_hours", employeeItem.getCurrent_fullday_hours());
		map.put("grace_early_time", employeeItem.getCurrent_grace_early_time());
		map.put("grace_late_time", employeeItem.getCurrent_grace_late_time());
		map.put("two_day_shift", employeeItem.isCurrent_two_day_shift());
	...
	public Map<String, Object> FnAddUpdateAttendanceDetails(@RequestParam("getEmployeeAttendanceDetails") JSONObject getEmployeeAttendanceDetails) {
		...
		try {
			List<CHtAttendanceMonthlyNewModel> htAttendanceMonthlyModels = new ArrayList<>();
			List<CHtAttendanceMonthlyJobTypeNewModel> htAttendanceMonthlyModelsJobTypeWise = new ArrayList<>();
...
//			CONVERT PUNCHING DETAILS DATA FROM JSON TO MODEL
			List<CHtAttendanceDailyNewModel> htAttendanceDailyModels = objectMapper.readValue(punchingDetailsData.toString(), new TypeReference<List<CHtAttendanceDailyNewModel>>() {
			});

			List<String> getEmployeeCodes = htAttendanceDailyModels.stream()
					.map(CHtAttendanceDailyNewModel::getEmployee_code).distinct().collect(Collectors.toList());

//			GET THE LEAVE APPLICATIONS OF EMPLOYEES
			List<Map<String, Object>> leaveApplicationDetails = iHmLeavesApplicationsRepository.getLeavesApplicationDetails(fromDate, toDate, getEmployeeCodes);

//			UPDATE EXISTING
			iHtAttendanceDailyNewRepository.updateExistingDetailsOfSameDate(getEmployeeCodes, dateList, attendanceRequests.getEmployee_type_group(), attendanceRequests.getCompany_id());


//			GET ATTENDANCE MONTHLY DETAILS
			List<CHtAttendanceMonthlyNewModel> getAttendanceMonthlyDetails = iHtAttendanceMonthlyNewRepository.getEmployeesMonthlyRecords(getEmployeeCodes, month, year);
...
//				            PREPARE CHTATTENDANCEMONTHLYMODEL TO STORE MONTHLY DATA OF EMPLOYEE ATTENDANCE
							Optional<CHtAttendanceMonthlyNewModel> findMatchingObjectOfMonth = getAttendanceMonthlyDetails.stream()
							...
							CHtAttendanceMonthlyNewModel htAttendanceMonthlyModel = new CHtAttendanceMonthlyNewModel();
...
//					            ADD ATTENDANCE STATUS IN OBJECT
								...
								if (htAttendanceMonthlyModel.getTotal_latemarks() != 0) {
									...
									CHtAttendanceMonthlyNewModel finalHtAttendanceMonthlyModel = htAttendanceMonthlyModel;
...
//					            GET MONTHLY ATTENDANCE FIELDS
								setPresentyDateWiseForMonthlyCalculations(empAtt, htAttendanceMonthlyModel, day, leaveApplicationDetails); // SET DATEWISE MONTHLY CALCULATION
...
//					            CALCULATE TOTAL HOURS
							...
							Optional<CHtAttendanceMonthlyNewModel> getMonthlyObjectFromExistingDetails = htAttendanceMonthlyModels.stream()
									.filter(item -> item.getEmployee_code().equals(empAtt.getEmployee_code())).findFirst();
...
//				Move to the next date
			...
			iHtAttendanceDailyNewRepository.saveAll(htAttendanceDailyModels);    //INSERT INTO HT_ATTENDANCE_DAILY EMPLOYEES DAILY ATTENDANCE

			List<CHtAttendanceMonthlyNewModel> getUpdatedMonthlyAttendance = iHtAttendanceMonthlyNewRepository.saveAll(htAttendanceMonthlyModels);  //INSERT INTO ht_attendance_monthly EMPLOYEES MONTHLY ATTENDANCE
...
//			    IN ACTIVE EXISTING SALARY JOB TYPE FOR THAT EMPLOYEES
				iHtAttendanceMonthlyJobTypeNewRepository.inActiveExistingJobTypeSalary(getEmployeeCodes, month, year);
...
//			    GET EMPLOYEE LATEST MONTH DATA FROM DAILY ATTENDANCE
				...
				String query = String.format(
						"SELECT model.*, emp.current_shift_start_end_time AS shift_start_end_time, emp.department_id, emp.sub_department_id " +
								"FROM ht_attendance_daily_new model " +
...
//          ITERATE ON EMPLOYEE CODE & PROCESS JOB TYPE WISE SALARY
				getEmployeeCodes.forEach(employeeCode -> {
					...
					Optional<CHtAttendanceMonthlyNewModel> findMatchingObjectOfMonth = getUpdatedMonthlyAttendance.stream()
					...
					List<CHtAttendanceMonthlyJobTypeNewModel> newEntries = new ArrayList<>();
					...
					getDailyAttendance.forEach(empAtt -> {
						...
						List<CHtAttendanceMonthlyJobTypeNewModel> currentEmpJobTypeWiseMonthlyDtl = newEntries.stream()
								.filter(item -> item.getEmployee_code().equals(empAtt.getEmployee_code()))
								.collect(Collectors.toList());

						// Create the new models for this employee's attendance
						List<CHtAttendanceMonthlyJobTypeNewModel> cHtAttendanceMonthlyJobTypeModels = FnSetMonthlyAttendaceJobTypeWise(
								empAtt, currentEmpJobTypeWiseMonthlyDtl, currentJobTypeCodeModel, findMatchingObjectOfMonth.get(), getDailyAttendance);
				...
				iHtAttendanceMonthlyJobTypeNewRepository.saveAll(htAttendanceMonthlyModelsJobTypeWise); //INSERT INTO ht_attendance_monthly EMPLOYEES JOBTypeCode MONTHLY ATTENDANCE
			}
	...
	public Optional<CHtvSalaryRulesViewModel> getApplicableRule(List<CHtvSalaryRulesViewModel> rules, Double lateMarks, CHtAttendanceDailyNewModel empAtt) {
	...
	private List<CHtAttendanceMonthlyJobTypeNewModel> FnSetMonthlyAttendaceJobTypeWise(CHtAttendanceDailyTempModel empAtt, List<CHtAttendanceMonthlyJobTypeNewModel> currentEmpJobTypeWiseMonthlyDtl, CJobTypeModel currentJobTypeModel,
	                                                                                   CHtAttendanceMonthlyNewModel htAttendanceMonthlyModel, List<CHtAttendanceDailyTempModel> getDailyAttendance) {
		List<CHtAttendanceMonthlyJobTypeNewModel> htAttendanceMonthlyModelsJobTypeWise = new ArrayList<>();
		Double totalShiftHrs = FnCalculateHoursInDecimal(empAtt.getShift_start_end_time());

		long totalAttendanceDaysOnJobCode = getDailyAttendance.stream()
				.filter(emp -> emp.getJob_type_code_id() == empAtt.getJob_type_code_id()
				&& emp.getPresent_flag().equals("Y"))
				.count();
...
//		Also Update that Day Salary into the ht_attendace_daily modal;
		CHtAttendanceMonthlyJobTypeNewModel existingJobTypeWiseMonthlyAtt = currentEmpJobTypeWiseMonthlyDtl.parallelStream()
...
//			);
			existingJobTypeWiseMonthlyAtt.setJob_type_total_salary(currentJobTypeModel.getJob_type_rate() * totalAttendanceDaysOnJobCode);
		...
		CHtAttendanceMonthlyJobTypeNewModel currentJobTypeMonthlyAtt = new CHtAttendanceMonthlyJobTypeNewModel();
...
//		);
		currentJobTypeMonthlyAtt.setJob_type_total_salary(currentJobTypeModel.getJob_type_rate() * 1);
	...
	private void updateAttendanceDays(CHtAttendanceDailyNewModel empAtt, CHtAttendanceMonthlyNewModel htAttendanceMonthlyModel,
...
//                      Check if the string contains '/'
						...
						switch (presentyString) {
							...
							case "HF":
								...
								if (shortLeaveStatus) {
									htAttendanceMonthlyModel.setHalf_days(htAttendanceMonthlyModel.getHalf_days() - 2);
								} else {
...
//          GET MISSED PUNCH DETAILS FROM THIS ht_attendance_daily
			List<CHtAttendanceDailyNewModel> getMissPunchDetails;
			if (attendanceToDate == null || attendanceToDate.isEmpty()) {
				getMissPunchDetails = iHtAttendanceDailyNewRepository.getMissPunchDetailsOnSingleDate(attendanceFromDate, getEmployeeCodes);
			} else {
				getMissPunchDetails = iHtAttendanceDailyNewRepository.getMissPunchDetailsWithinDateRange(attendanceFromDate, attendanceToDate, getEmployeeCodes);
			}
...
//								.collect(Collectors.toList());
						...
						List<CHtAttendanceDailyNewModel> getMissPunchDetailsEmployeeWise = getMissPunchDetails.stream()
								.filter(item -> item.getEmployee_code().equals(employeeItem.getEmployee_code()))
								.filter(item ->
						...
						getMissPunchDetailsEmployeeWise.forEach(missingPunchItem -> {
							...
							employeeMap.put("shift_name", employeeItem.getShift_name());
