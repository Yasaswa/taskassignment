...
select
    ...
    `mt`.`remark` as `remark`,
      case
        `mt`.`dispatch_challan_status` when 'D' then 'Dispatched'
        else 'Pending'
...
select
    ...
    concat(ifnull(`v`.`dispatch_challan_status_desc`, ''), ':Dispatch Challan Status:Y:T:') as `dispatch_challan_status_desc`,
