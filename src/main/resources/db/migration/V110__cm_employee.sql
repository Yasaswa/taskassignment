...
select
    ...
    concat(`mss`.`start_time`, '-', `mss`.`end_time`) as `current_shift_start_end_time`,
    `mss`.`halfday_hours` as `current_halfday_hours`,
    `mss`.`fullday_hours` as `current_fullday_hours`,
    `mss`.`grace_early_time` as `current_grace_early_time`,
    `mss`.`grace_late_time` as `current_grace_late_time`,
    ...
    ifnull(`mss`.`two_day_shift`, 0) as `current_two_day_shift`,
    ...
    `mss`.`shift_grace_hours_min` as `current_shift_grace_hours_min`,
    `mss`.`shift_grace_hours_max` as `current_shift_grace_hours_max`,
