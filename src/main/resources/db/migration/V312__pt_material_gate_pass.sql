ALTER TABLE pt_material_gate_pass_master ADD supplier_branch_id bigint(20) NULL;
ALTER TABLE pt_material_gate_pass_master ADD supplier_branch_state_id bigint(20) NULL;
ALTER TABLE pt_material_gate_pass_master ADD supplier_branch_city_id bigint(20) NULL;
ALTER TABLE pt_material_gate_pass_master ADD supplier_branch_contacts_id bigint(20) NULL;
ALTER TABLE pt_material_gate_pass_master CHANGE service_provider_add supplier_branch_address varchar(350) DEFAULT NULL;

ALTER TABLE pt_material_gate_pass_return_summary ADD return_gate_pass_no varchar(50) NOT NULL;

ALTER TABLE pt_material_gate_pass_master ADD return_status varchar(3) NULL;

ALTER TABLE pt_material_gate_pass_details ADD return_item_status varchar(3) NULL;
...
where
    ...
    select
        ...
        concat(ifnull(`v`.`gate_pass_status_desc`, ''), ':Gate Pass Status:Y:H:(Approved,Pending)') as `gate_pass_status_desc`,
        concat(ifnull(`v`.`product_type_name`, ''), ':Product Type Name:Y:T:') as `product_type_name`,
        concat(ifnull(`v`.`material_type_desc`, ''), ':Material Type:Y:H:(Returnable,Non-Returnable)') as `material_type_desc`,
        concat(ifnull(`v`.`vehicle_no`, ''), ':Vehicle No:Y:T:') as `vehicle_no`,
        concat(ifnull(`v`.`supplier_branch_address`, ''), ':Supplier Address:O:N:') as `supplier_branch_address`,
