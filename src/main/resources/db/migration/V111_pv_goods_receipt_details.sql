-- erp_development_temp.ptv_goods_receipt_details source
...
select
    ...
    `po`.`sub_department_id` as `sub_department_id`,
    `po`.`cost_center_id` as `cost_center_id`,
...
where
    ...
    -- erp_development_temp.ptv_goods_receipt_details_rpt source
    ...
    select
        ...
        concat(ifnull(`v`.`cost_center_name`, ''), ':Cost Center Name:Y:C:fmv_cost_center:F') as `cost_center_name`,
