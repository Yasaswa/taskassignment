...
select
    ...
    ifnull((select `ptdetails`.`closing_no_of_boxes` from `sm_product_rm_stock_details` `ptdetails` where `ptdetails`.`goods_receipt_no` = `st`.`goods_receipt_no` and `ptdetails`.`batch_no` = `st`.`issue_batch_no` and `ptdetails`.`day_closed` = 0), 0) as `closing_no_of_boxes`,
