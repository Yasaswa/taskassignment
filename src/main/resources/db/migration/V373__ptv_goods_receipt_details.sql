...
select
    ...
    `comm`.`product_rm_std_weight` as `product_material_std_weight`,
...
left join `sm_product_rm` `rm` on
 ...
 left join `sm_product_category1` `cat1` on
    (`rm`.`product_rm_id` = `pt`.`product_material_id`
        and `cat1`.`product_category1_id` = `rm`.`product_category1_id`
        and `cat1`.`is_delete` = 0))
left join `sm_product_rm_commercial` `comm` on
    (`comm`.`product_rm_id` = `pt`.`product_material_id`
        and `comm`.`is_delete` = 0))
left join `sm_product_rm_technical` `tech` on
    (`tech`.`product_rm_id` = `pt`.`product_material_id`
