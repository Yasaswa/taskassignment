...
-- erp.stv_indent_master_summary source
...
order by
    ...
    select
        ...
        case
            `v`.`grn_status` when 'A' then 'Approved'
            when 'Q' then 'QC Done'
            when 'R' then 'Rejected'
            when 'I' then 'Partial Receipt'
            when 'C' then 'Completed'
...
select
    ...
    concat(ifnull(`v`.`grn_status_desc`, ''), ':GRN Status:Y:H:(Pending, Goods Receipts,Partial Receipt,GRN Done)') as `grn_status_desc`,
...
-- erp.stv_indent_master_summary_rpt source
