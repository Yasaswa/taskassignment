ALTER TABLE xt_weaving_production_inspection_details ADD `style` varchar(155) NULL;
...
create or replace
algorithm = UNDEFINED view `xtv_weaving_production_inspection_wastage` as
select
    `im`.`inspection_production_code` as `inspection_production_code`,
    `iw`.`shift` as `shift`,
    `im`.`prod_month` as `prod_month`,
    `im`.`prod_year` as `prod_year`,
    `im`.`inspection_production_date` as `inspection_production_date`,
    `im`.`plant_name` as `plant_name`,
    `s`.`godown_name` as `godown_name`,
    `im`.`production_sub_section_name` as `production_sub_section_name`,
    `im`.`production_section_name` as `production_section_name`,
    ...
    `iw`.`production_wastage_types_type` as `production_wastage_types_type`,
    `iw`.`wastage_quantity` as `wastage_quantity`,
    case
        `iw`.`is_active` when 1 then 'Active'
        else 'In Active'
    end as `Active`,
    case
        `iw`.`is_delete` when 1 then 'Yes'
        else 'No'
    end as `Deleted`,
    `im`.`is_active` as `is_active`,
    `im`.`is_delete` as `is_delete`,
    `im`.`created_by` as `created_by`,
    `im`.`created_on` as `created_on`,
    `im`.`modified_by` as `modified_by`,
    `im`.`modified_on` as `modified_on`,
    `im`.`deleted_by` as `deleted_by`,
    `im`.`deleted_on` as `deleted_on`,
    `im`.`company_name` as `company_name`,
    `im`.`company_branch_name` as `company_branch_name`,
    `im`.`financial_year` as `financial_year`,
    `im`.`company_id` as `company_id`,
    `im`.`company_branch_id` as `company_branch_id`,
    `iw`.`weaving_production_inspection_master_id` as `weaving_production_inspection_master_id`,
    `iw`.`weaving_production_inspection_wastage_id` as `weaving_production_inspection_wastage_id`,
    `im`.`plant_id` as `plant_id`,
    `im`.`section_id` as `section_id`,
    `iw`.`godown_id` as `godown_id`,
    `im`.`sub_section_id` as `sub_section_id`,
    `iw`.`machine_id` as `machine_id`,
    `iw`.`production_wastage_types_id` as `production_wastage_types_id`
from
    ((((`xt_weaving_production_inspection_wastage` `iw`
left join `xtv_weaving_production_inspection_master` `im` on
    (`iw`.`weaving_production_inspection_master_id` = `im`.`weaving_production_inspection_master_id`
        and `iw`.`company_id` = `im`.`company_id`))
left join `xmv_production_sub_section_godown_mapping` `s` on
    (`s`.`production_sub_section_godown_id` = `iw`.`godown_id`
        and `s`.`company_id` = `iw`.`company_id`))
left join `cmv_machine` `cm` on
    (`iw`.`machine_id` = `cm`.`machine_id`
        and `iw`.`company_id` = `cm`.`company_id`))
left join `xmv_production_wastage_types` `wt` on
    (`iw`.`production_wastage_types_id` = `wt`.`production_wastage_types_id`
        and `iw`.`company_id` = `wt`.`company_id`))
where
    `iw`.`is_delete` = 0
order by
    `iw`.`weaving_production_inspection_wastage_id` desc;