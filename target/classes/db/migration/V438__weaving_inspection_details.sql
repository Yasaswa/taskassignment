ALTER TABLE xt_weaving_production_inspection_details CHANGE production_operator_id production_operator_name VARCHAR(155) DEFAULT 0 NULL COMMENT '*  grid data entry  combo box with cmv_employee_list ';
ALTER TABLE xt_weaving_production_inspection_details MODIFY COLUMN production_operator_name VARCHAR(155) DEFAULT 0 NULL COMMENT '*  grid data entry  combo box with cmv_employee_list ';
...
select
    ...
    concat(ifnull(`v`.`product_material_name`, ''), ':Product Name:Y:T:') as `product_material_name`,
        concat(ifnull(`v`.`production_operator_name`, ''), ':Checker Name:Y:T:') as `production_operator_name`,
