...
where
    `xt`.`is_delete` = 0
order by warping_production_order_id desc;





-- VIEW  WARPING PRODUCTION - MASTER
...
where
    `xt`.`is_delete` = 0
    order by
    `xt`.`weaving_production_sizing_master_id` desc;
...
--  RPT WARPING PRODUCTION - DETAILS
