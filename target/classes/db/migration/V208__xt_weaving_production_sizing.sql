ALTER TABLE xt_weaving_production_sizing_master ADD customer_order_no varchar(500) NOT NULL;
ALTER TABLE xt_weaving_production_sizing_master ADD set_no varchar(100) NOT NULL;
ALTER TABLE xt_beam_inwards_table ADD customer_order_no varchar(500) ;
ALTER TABLE xt_weaving_production_sizing_details ADD customer_order_no varchar(500) NOT NULL;
ALTER TABLE xt_sizing_production_stock_details ADD customer_order_no varchar(500) NOT NULL;


-- erp_development.xtv_weaving_production_sizing_master source

create or replace
algorithm = UNDEFINED view `xtv_weaving_production_sizing_master` as
select
    ...
    case
        `xt`.`sizing_production_master_status` when 'P' then 'Pending'
        when 'A' then 'Approved'
        when 'R' then 'Rejected'
        when 'C' then 'Completed'
        when 'X' then 'Canceled'
        else 'Partial Approved'
    end as `sizing_production_master_status_desc`,
    `xt`.`sizing_production_master_status` as `sizing_production_master_status`,
    case
    ...
    (
    select
        ifnull(sum(`sm`.`sizing_length`), 0)
    from
        `xtv_weaving_production_sizing_details` `sm`
    where
        `sm`.`sizing_production_code` = `xt`.`sizing_production_code`
        and `sm`.`is_delete` = 0) as `total_sizing_length`,
    (
    select
        ifnull(sum(`sm1`.`warping_length`), 0)
    from
        `xtv_weaving_production_sizing_details` `sm1`
    where
        `sm1`.`sizing_production_code` = `xt`.`sizing_production_code`
        and `sm1`.`is_delete` = 0) as `total_warping_length`,
    (
    select
        ifnull(sum(`sm2`.`gross_weight`), 0)
    from
        `xtv_weaving_production_sizing_details` `sm2`
    where
        `sm2`.`sizing_production_code` = `xt`.`sizing_production_code`
        and `sm2`.`is_delete` = 0) as `total_gross_weight`,
    (
    select
        ifnull(sum(`sm3`.`tare_weight`), 0)
    from
        `xtv_weaving_production_sizing_details` `sm3`
    where
        `sm3`.`sizing_production_code` = `xt`.`sizing_production_code`
        and `sm3`.`is_delete` = 0) as `total_tare_weight`,
    (
    select
        ifnull(sum(`sm4`.`net_weight`), 0)
    from
        `xtv_weaving_production_sizing_details` `sm4`
    where
        `sm4`.`sizing_production_code` = `xt`.`sizing_production_code`
        and `sm4`.`is_delete` = 0) as `total_net_weight`,
    (
    select
        ifnull(sum(`sm5`.`unsize_beam_weight`), 0)
    from
        `xtv_weaving_production_sizing_details` `sm5`
    where
        `sm5`.`sizing_production_code` = `xt`.`sizing_production_code`
        and `sm5`.`is_delete` = 0) as `total_unsize_beam_weight`,
    (
    select
        ifnull(sum(`sm6`.`size_pickup`), 0)
    from
        `xtv_weaving_production_sizing_details` `sm6`
    where
        `sm6`.`sizing_production_code` = `xt`.`sizing_production_code`
        and `sm6`.`is_delete` = 0) as `total_size_pickup`,
    (
    select
        ifnull(sum(`sm7`.`size_waste`), 0)
    from
        `xtv_weaving_production_sizing_details` `sm7`
    where
        `sm7`.`sizing_production_code` = `xt`.`sizing_production_code`
        and `sm7`.`is_delete` = 0) as `total_size_waste`,
    (
    select
        ifnull(sum(`sm8`.`unsize_waste`), 0)
    from
        `xtv_weaving_production_sizing_details` `sm8`
    where
        `sm8`.`sizing_production_code` = `xt`.`sizing_production_code`
        and `sm8`.`is_delete` = 0) as `total_unsize_waste`,
    (
    select
        ifnull(sum(`sm9`.`creel_waste`), 0)
    from
        `xtv_weaving_production_sizing_details` `sm9`
    where
        `sm9`.`sizing_production_code` = `xt`.`sizing_production_code`
        and `sm9`.`is_delete` = 0) as `total_creel_waste`,
    (
    select
        ifnull(sum(`sm10`.`rate`), 0)
    from
        `xtv_weaving_production_sizing_details` `sm10`
    where
        `sm10`.`sizing_production_code` = `xt`.`sizing_production_code`
        and `sm10`.`is_delete` = 0) as `total_rate`,
    (
    select
        ifnull(sum(`sm11`.`amount`), 0)
    from
        `xtv_weaving_production_sizing_details` `sm11`
    where
        `sm11`.`sizing_production_code` = `xt`.`sizing_production_code`
        and `sm11`.`is_delete` = 0) as `total_amount`,
    ...
    `xt`.`weaving_production_sizing_stoppage_id` as `weaving_production_sizing_stoppage_id`,
    ...
    `xt`.`weaving_production_sizing_master_id` as `field_id`,
    `xt`.`sizing_production_code` as `field_name`
from
    ((((`xt_weaving_production_sizing_master` `xt`
