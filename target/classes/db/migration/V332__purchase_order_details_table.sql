...
select
    ...
    coalesce((select sum(`ptcdetails`.`product_material_grn_accepted_quantity`) from `pt_grn_cotton_bales_details` `ptcdetails` where `ptcdetails`.`purchase_order_details_transaction_id` = `pt`.`purchase_order_details_transaction_id` and `ptcdetails`.`is_delete` = 0 and `ptcdetails`.`company_id` = `pt`.`company_id`), 0) as `prev_grn_bales_quantity`,
    coalesce((select sum(`ptcdetails`.`product_material_grn_accepted_weight`) from `pt_grn_cotton_bales_details` `ptcdetails` where `ptcdetails`.`purchase_order_details_transaction_id` = `pt`.`purchase_order_details_transaction_id` and `ptcdetails`.`is_delete` = 0 and `ptcdetails`.`company_id` = `pt`.`company_id`), 0) as `prev_grn_bales_weight`
from
