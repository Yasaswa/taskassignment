ALTER TABLE xt_weaving_production_inspection_master ADD inspection_production_set_no varchar(255) NOT NULL;
ALTER TABLE xt_weaving_production_inspection_details ADD product_material_name varchar(555) NULL;
ALTER TABLE xt_weaving_production_inspection_details DROP COLUMN total_no_of_ends;
ALTER TABLE xt_weaving_production_inspection_details CHANGE product_id product_material_id varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL NULL;
...
select
    concat(ifnull(`v`.`inspection_production_code`, ''), ':Inspection Production Code:O:N:') as `inspection_production_code`,
    concat(ifnull(`v`.`inspection_production_date`, ''), ':Inspection Production Date:Y:D:') as `inspection_production_date`,
    ...
    concat(ifnull(`v`.`inspection_production_master_status`, ''), ':Inspection Production Master Status Desc:Y:H:(Pending,Approved,Rejected,Completed,Canceled,Partial Approved)') as `inspection_production_master_status_desc`,
    ...
    concat(ifnull(`v`.`inspection_production_master_status`, ''), ':inspection Production Master Status:O:N:') as `inspection_production_master_status`,
    ...
    concat(ifnull(`v`.`weaving_production_inspection_master_id`, ''), ':Weaving Production Inspection Master Id:O:N:') as `weaving_production_inspection_master_id`,
...
left join `xt_warping_production_order` `xwpo` on
    (`xwpo`.`set_no` = `xt`.`inspection_production_set_no` and `xwpo`.`is_delete` = 0))
left join `mt_sales_order_master_trading` `msomt` on
    (`msomt`.`customer_order_no` = `xwpo`.`customer_order_no` and `msomt`.`is_delete` = 0 and `msomt`.`company_id` = `xwpo`.`company_id`))
where
    `xt`.`is_delete` = 0;
...
select
    concat(ifnull(`v`.`product_material_name`, ''), ':SO Item Name:O:N:') as `product_material_name`,
    concat(ifnull(`v`.`inspection_production_set_no`, ''), ':Set No:O:N:') as `inspection_production_set_no`,
    concat(ifnull(`v`.`inspection_production_date`, ''), ':Production Date:Y:D:') as `inspection_production_date`,
    concat(ifnull(`v`.`inspection_production_code`, ''), ':Production Code:Y:C:xtv_weaving_production_inspection_details:O') as `inspection_production_code`,
    ...
    concat(ifnull(`v`.`roll_no`, ''), ':Roll No:O:N:') as `roll_no`,
    ...
    concat(ifnull(`v`.`inspection_mtr`, ''), ':Inspection Mtr:O:N:') as `inspection_mtr`,
    ...
    concat(ifnull(`v`.`inspection_production_master_status`, ''), ':Inspection Master Status Desc:Y:H:(Pending,Approved,Rejected,Completed,Canceled,Partial Approved)') as `inspection_production_master_status_desc`,
    concat(ifnull(`v`.`production_section_name`, ''), ':Production Section Name:O:N:') as `production_section_name`,
    ...
    concat(ifnull(`v`.`inspection_production_status`, ''), ':Inspection Production Status Desc:Y:H:(Pending,Approved,Rejected,Completed,Canceled,Partial,Partial Dispatch,Dispatch Done)') as `inspection_production_status_desc`,
    concat(ifnull(`v`.`status_remark`, ''), ':Status Remark:O:N:)') as `status_remark`,
    concat(ifnull(`v`.`inspection_production_status`, ''), ':Inspection Status:O:N:)') as `inspection_production_status`,
    concat(ifnull(`v`.`company_name`, ''), ':Company Name:Y:C:cmv_company_summary:F') as `company_name`,
    concat(ifnull(`v`.`company_branch_name`, ''), ':Company Branch Name:Y:C:cmv_company_branch_summary:F') as `company_branch_name`,
    concat(ifnull(`v`.`financial_year`, ''), ':Financial Year:Y:C:amv_financial_year:F') as `financial_year`,
    concat(ifnull(`v`.`inspection_production_master_status`, ''), ':Inspection Master Status:O:N:)') as `inspection_production_master_status`,
    concat(case when `v`.`is_active` = 1 then 'Active' else 'In Active' end, ':Active Status:Y:H:(Active, In Active)') as `Active`,
    concat(case when `v`.`is_delete` = 1 then 'Yes' else 'No' end, ':Deleted Status:Y:H:(Yes, No)') as `Deleted`,
    concat(ifnull(`v`.`created_by`, ''), ':Created By:O:N:') as `created_by`,
    concat(ifnull(`v`.`created_on`, ''), ':Created On:O:N:') as `created_on`,
    ...
    concat(ifnull(`v`.`company_id`, ''), ':Company Id:N:N:') as `company_id`,
    concat(ifnull(`v`.`company_branch_id`, ''), ':Company Branch Id:N:N:') as `company_branch_id`,
    concat(ifnull(`v`.`weaving_production_inspection_details_id`, ''), ':Weaving Production Inspection Details Id:N:N:') as `weaving_production_inspection_details_id`,
    ...
    concat(ifnull(`v`.`product_material_id`, ''), ':Product Id:N:N:') as `product_material_id`,
    concat(ifnull(`v`.`plant_id`, ''), ':Plant Id:N:N:') as `plant_id`
from
