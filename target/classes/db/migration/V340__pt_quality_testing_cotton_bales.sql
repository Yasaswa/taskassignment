CREATE TABLE `pt_quality_testing_cotton_bales` (
  `quality_testing_cotton_bales_transaction_id` bigint(20) NOT NULL AUTO_INCREMENT,
  ...
  `unit_testing_no` varchar(50) NOT NULL,
  `unit_testing_date` date NOT NULL,
  ...
  `quality_tester_id` bigint(20) NOT NULL,
  ...
  PRIMARY KEY (`quality_testing_cotton_bales_transaction_id`)
);
...
select
    ...
    `qt`.`unit_testing_no` as `unit_testing_no`,
    `qt`.`financial_year` as `financial_year`,
    `qt`.`unit_testing_date` as `unit_testing_date`,
    ...
    `qt`.`unit_testing_no` as `field_name`
from
...
select
    concat(`v`.`unit_testing_no`, ':Unit Testing No:O:N:') as `unit_testing_no`,
    concat(`v`.`unit_testing_date`, ':Unit Testing Date:Y:D:') as `unit_testing_date`,
    concat(`v`.`quality_testing_status_desc`, ':Quality Testing Status:Y:T:') as `quality_testing_status_desc`,
